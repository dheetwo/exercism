...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    percentage_value = (voltage * current) * 100 / theoretical_max_power
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    if temperature * neutrons_produced_per_second < 0.9 * threshold:
        return 'LOW'
    elif temperature * neutrons_produced_per_second - 0.1 * threshold <= threshold <= temperature * neutrons_produced_per_second + 0.1 * threshold:
        return 'NORMAL'
    else:
        return 'DANGER'
